      ...
      <div className="relative z-10">
        <div className="container mx-auto px-4 py-6 pb-24 md:px-6 md:py-16 md:pb-16">
          <div className="flex flex-col gap-8 md:gap-12">
            ...
            {/* Sezione Evento Attivo - Condizionale */}
            {selectedEvent && selectedEventId && (
              <Card className="glass border-2 border-primary/40 bg-gradient-to-br from-primary/10 via-background to-accent/10">
                <CardHeader>
                  <div className="flex items-start justify-between gap-4">
                    <div className="space-y-2">
                    ...
                    <div className={`rounded-full px-3 py-1 text-xs font-semibold ${
                      selectedEvent.status === 'PUBLISHED' ? 'bg-emerald-500/20 text-emerald-400' :
                      selectedEvent.status === 'DRAFT' ? 'bg-yellow-500/20 text-yellow-400' :
                      selectedEvent.status === 'CANCELLED' ? 'bg-red-500/20 text-red-400' :
                      'bg-blue-500/20 text-blue-400'
                    }`}>
                      {selectedEvent.status === 'PUBLISHED' ? 'Pubblicato' :
                       selectedEvent.status === 'DRAFT' ? 'Bozza' :
                       selectedEvent.status === 'CANCELLED' ? 'Annullato' : selectedEvent.status}
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        >
                          <Link href={`/eventi/${selectedEventId}`}>
                        ...
                        >
                          <Link href={`/analytics/${selectedEventId}`}>
                        ...
                        <Button
                          asChild
                          variant="outline"
                          className="glass border-orange-500/40 justify-start"
                        >
                          <Link href={`/eventi/${selectedEventId}/settings/complimentary`}>
                        ...
                        <Button
                          asChild
                          variant="outline"
                          className="glass border-pink-500/40 justify-start"
                        >
                          <Link href={`/eventi/${selectedEventId}/consumazioni`}>
                        ...
                        <Button
                          asChild
                          variant="outline"
                          className="glass border-cyan-500/40 justify-start"
                        >
                          <Link href={`/checkin?eventId=${selectedEventId}`}>
            ...
            {/* Messaggio quando nessun evento è selezionato */}
            {!selectedEvent && (
              <Card className="glass border border-dashed border-muted-foreground/30">
                <CardContent className="py-8 text-center">
                  <div className="mx-auto w-fit rounded-full bg-muted/50 p-4 mb-4">
                    <CalendarDays className="h-8 w-8 text-muted-foreground" />
                  </div>
                  <p className="text-muted-foreground">
                    Seleziona un evento per visualizzare azioni contestuali e strumenti di gestione rapidi
                  </p>
                </CardContent>
              </Card>
            )}
