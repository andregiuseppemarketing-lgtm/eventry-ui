...
export function MarketingFunnelPageClient() {
  const { data: session, status } = useSession();
  const router = useRouter();
  const { selectedEventId: globalEventId } = useEventContext();
  const [events, setEvents] = useState<any[]>([]);
  const [funnelEventId, setFunnelEventId] = useState<string>('');
    ...
    const fetchEvents = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (eventsArray.length > 0 && !funnelEventId) {
            setFunnelEventId(eventsArray[0].id);
          }
        } else {
  ...
  const fetchFunnelData = useCallback(async () => {
    if (!funnelEventId) return;

    setLoading(true);
    try {
      const res = await fetch(`/api/funnel/events/${funnelEventId}`);
      if (res.ok) {
        const data = await res.json();
        setFunnelData(data);
      }
    ...
    <div className="min-h-screen bg-gradient-to-b from-black via-purple-950/20 to-black p-3 sm:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-4 sm:mb-6">
          ...
          <select
            value={funnelEventId}
            onChange={(e) => setFunnelEventId(e.target.value)}
